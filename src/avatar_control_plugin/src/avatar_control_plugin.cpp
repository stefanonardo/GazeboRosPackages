...
namespace gazebo {
  ...
  void AvatarControlPlugin::OnUpdate(const common::UpdateInfo & /*_info*/) {
    ...
    /*math::Pose model_world_pose = this->model_->WorldPose();
    ...
    math::Quaternion rotation_diff_quat = this->model_target_rotation_ * model_world_pose.Rot().Inverse();
...
//////////////////////////////////////// Update functions called during OnUpdate() ////////////////////////////////
  ...
  void AvatarControlPlugin::UpdateModelVelocitiesFromModelPoseTarget()
  {
    ...
    //TODO: have to get the parent/base link, setting model velocities resets joint forces
    // make more generic with controller and the base link name
    physics::LinkPtr link_hips = this->model_->GetLink("avatar_ybot::mixamorig_Hips");
    if (link_hips != nullptr)
    {
      //ROS_INFO("UpdateModelVelocitiesFromModelPoseTarget() - %s", link_hips->GetName().c_str());
      ...
      link_hips->SetLinearVel(linear_velocity);
      ...
      link_hips->SetAngularVel(angular_velocity);
    }
